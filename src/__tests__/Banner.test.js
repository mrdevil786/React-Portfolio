...
test('it shows the title in the banner', () => {
	setup();
	// We expect that the title 'Hi, I am Suhail Saeed Usmani' is in the banner component
	expect(screen.getByText(/Hi, I am Suhail Saeed Usmani/i)).toBeInTheDocument();
});
...
test('can download cv when clicked on download cv button', async () => {
	...
	// expect(downloadLink.download).toEqual('Suhail-Resume.pdf');
	// expect(downloadLink.href).toEqual('/files/Suhail-Resume.pdf');
